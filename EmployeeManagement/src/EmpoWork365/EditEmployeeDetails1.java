 ...
 * @author jenal
 */
public final class EditEmployeeDetails1 extends javax.swing.JFrame {
     ...
     * @param mainAdmin
     */
    public EditEmployeeDetails1(MainAdmin mainAdmin) {
    ...
    private void setupJobTitleComboBox() {
        ...
        DefaultComboBoxModel<JobTitle> model = new DefaultComboBoxModel<>(jobTitles.toArray(JobTitle[]::new));

        comboBoxJobTitle.setModel(model);
        comboBoxJobTitle.setEditable(false); 
        ...
        jobTitleTextField.addKeyListener(new KeyAdapter() {
            ...
            public void keyReleased(KeyEvent e) {
                String input = jobTitleTextField.getText().toLowerCase(); 
                DefaultComboBoxModel<JobTitle> filteredModel = new DefaultComboBoxModel<>();

                for (JobTitle jobTitle : jobTitles) {
                    if (jobTitle.getTitle().toLowerCase().contains(input)) {
                        filteredModel.addElement(jobTitle);
                    }
                }

                comboBoxJobTitle.setModel(filteredModel); 
                comboBoxJobTitle.setSelectedItem(null); 
                jobTitleTextField.setText(input);
                jobTitleTextField.setCaretPosition(input.length()); 
                comboBoxJobTitle.showPopup(); 
            }
        });
    ...
    public int getSelectedJobTitleId() {
        ...
        if (selectedItem instanceof JobTitle) {
            return ((JobTitle) selectedItem).getId(); 
        } else {
   ...
   private void initializeRoleComboBox() {
    ...
    private boolean isDataUnchanged(int employeeId) {
    ...
    return firstName.getText().trim().isEmpty() || 
           ...
           comboBoxDepartment.getSelectedItem() == null ||
           comboBoxRole.getSelectedItem() == null ||
           imageLabel.getText().trim().isEmpty(); 
    }
    ...
    private void btnConfirmEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnConfirmEditActionPerformed
        ...
        // Prepare SQL statement and target ID
        String updateEmployeeSQL = "UPDATE tbl_employees SET fld_first_name = ?, fld_last_name = ?, fld_email = ?, "
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(updateEmployeeSQL)) {
            ...
            pstmt.setString(9, imageLocation != null ? imageLocation : ""); // Handle optional image
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(EditEmployeeDetails1.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                ...
                new EditEmployeeDetails1(mainAdmin).setVisible(true);
            }
        });
    }
